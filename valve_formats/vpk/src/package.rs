...
use crc::{self, Crc};
use io_util::{PrimitiveRead, RawDataRead, StringRead, StringReadError};
use rsa::{BigUint, Pkcs1v15Encrypt};
use std::collections::HashMap;
use std::io::{BufReader, Error as IOError, Read, Result as IOResult, Seek, SeekFrom};
use std::sync::Mutex;
    ...
    R: Read + Seek,
{
    ...
    pub fn read<F: Send + Sync + Fn(&str) -> IOResult<R>>(
        file_name: &str,
        mut input: R,
        open_file_callback: F,
    ) -> Result<Self, PackageError> {
        let (file_name, is_dir_vpk) = Self::sanitize_file_name(file_name);

        if input.read_u32().map_err(PackageError::IOError)? != MAGIC {
            return Err(PackageError::FileError(
                "Given file is not a VPK.".to_string(),
            ));
        }
        ...
        let (
            ...
            return Err(PackageError::FileError(format!(
                "Bad VPK version: {}",
                version
            )));
        };
        ...
        let header_size = input
            ...
            .map_err(PackageError::IOError)? as u32;
        ...
        for (_name, entries) in &entries {
            for entry in entries {
                ...
                archive_files.insert(
                    entry.archive_index,
                    BufReader::new(file.map_err(PackageError::IOError)?),
                );
            }
        }
        ...
        let (
            ...
            input
                .seek(SeekFrom::Current(file_data_section_size as i64))
                .map_err(PackageError::IOError)?;
            let archive_md5_entries =
                Self::read_archive_md5_section(&mut input, archive_md5_section_size)?;
            let (tree_checksum, archive_md5_entries_checksum, whole_file_checksum) =
                Self::read_other_md5_section(&mut input, other_md5_section_size)?;
            let (public_key, signature) =
                Self::read_signature_section(&mut input, signature_section_size)?;
            (
                archive_md5_entries,
                tree_checksum,
                archive_md5_entries_checksum,
    ...
    pub fn find_entry(&self, file_path: &str) -> Option<&PackageEntry> {
        ...
        let (file_name, directory) = if let Some(last_separator) = last_separator {
            (
                &file_path[last_separator + 1..],
                &file_path[..last_separator],
            )
        } else {
            (file_path.as_str(), "")
        };
    ...
    pub fn find_entry_in_dir_with_extension(
        &self,
        directory: &str,
        file_name: &str,
        file_extension: &str,
        ...
        self.entries[file_extension].iter().find(|x| {
            x.directory_name.as_str() == trimmed_directory && x.file_name.as_str() == file_name
        })
    }
    ...
    pub fn read_entry(
        ...
        if entry.len > 0 {
            if entry.archive_index != 0x7FFF {
                if !self.is_dir_vpk {
                    return Err(PackageError::FileError(
                        "Given VPK is not a _dir, but entry is referencing an external archive."
                            .to_string(),
                    ));
                }
                ...
                file.read_exact(
                    &mut output
                        [entry.small_data.len()..entry.small_data.len() + entry.len as usize],
                )
                .map_err(PackageError::IOError)?;
            } else {
        ...
        if validate_crc && entry.crc32 != crc.checksum(&output) {
            return Err(PackageError::FileError(
                "CRC32 mismatch for read data.".to_string(),
            ));
        }
    ...
    fn read_entries(input: &mut R) -> Result<HashMap<String, Vec<PackageEntry>>, PackageError> {
        ...
        'types: loop {
            let type_name = input.read_null_terminated_string().map_err(|e| match e {
                ...
                StringReadError::StringConstructionError(_) => {
                    PackageError::FileError("Failed to read type name".to_string())
                }
            })?;
            ...
            'entries: loop {
                ...
                'files: loop {
                    ...
                    if small_data_len > 0 {
                        entry.small_data = input
                            .read_data(small_data_len)
                            .map_err(PackageError::IOError)?;
                    }
    ...
    pub fn verify_hashes(&self) -> Result<(), PackageError> {
        if self.version != 2 {
            return Err(PackageError::FileError(
                "Only version 2 is supported.".to_string(),
            ));
        }

        {
            ...
            if hash.0 != self.whole_file_checksum {
                return Err(PackageError::FileError(format!(
                    "Package checksum mismatch ({:?} != expected {:?}).",
                    &hash, &self.whole_file_checksum
                )));
            }

            reader
                .seek(SeekFrom::Start(
                    (self.header_size + self.tree_size + self.file_data_section_size) as u64,
                ))
                .map_err(PackageError::IOError)?;
            reader
                .read_exact(&mut buffer[..self.archive_md5_section_size as usize])
                .map_err(PackageError::IOError)?;
            hash = md5::compute(&buffer[..self.archive_md5_section_size as usize]);
            if hash.0 != self.whole_file_checksum {
                return Err(PackageError::FileError(format!(
                    "Archive MD5 entries checksum mismatch ({:?} != expected {:?}).",
                    &hash, &self.archive_md5_entries_checksum
                )));
            }
        ...
        if !self.is_signature_valid() {
            return Err(PackageError::FileError(
                "VPK signature is not valid.".to_string(),
            ));
        }
    ...
    pub fn is_signature_valid(&self) -> bool {
        ...
        let public_key_res = rsa::RsaPublicKey::new(
            BigUint::from_bytes_le(&parameters.modulus),
            BigUint::from_bytes_le(&parameters.exponent),
        );
        if public_key_res.is_err() {
            return false;
        }
        let public_key = public_key_res.unwrap();
        let data_res = reader.read_data(
            (self.header_size
                + self.tree_size
                + self.file_data_section_size
                + self.archive_md5_section_size
                + self.other_md5_section_size) as usize,
        );
        if data_res.is_err() {
            return false;
        }
    ...
    fn read_archive_md5_section(
        input: &mut R,
        archive_md5_section_size: u32,
    ) -> Result<Vec<ArchiveMD5SectionEntry>, PackageError> {
        ...
        let entries =
            archive_md5_section_size / std::mem::size_of::<ArchiveMD5SectionEntry>() as u32;
    ...
    fn read_other_md5_section(
        input: &mut R,
        other_md5_section_size: u32,
    ) -> Result<([u8; 16], [u8; 16], [u8; 16]), PackageError> {
        if other_md5_section_size != 48 {
            return Err(PackageError::FileError(format!(
                "Encountered OtherMD5Section with size of {} (should be 48)",
                other_md5_section_size
            )));
        }
        ...
        input
            .read_exact(&mut tree_checksum)
            .map_err(PackageError::IOError)?;
        let mut archive_md5_entries_checksum = [0u8; 16];
        input
            .read_exact(&mut archive_md5_entries_checksum)
            .map_err(PackageError::IOError)?;
        let mut whole_file_checksum = [0u8; 16];
        input
            .read_exact(&mut whole_file_checksum)
            .map_err(PackageError::IOError)?;
        Ok((
            tree_checksum,
            archive_md5_entries_checksum,
    ...
    fn read_signature_section(
        input: &mut R,
        signature_section_size: u32,
    ) -> Result<(Box<[u8]>, Box<[u8]>), PackageError> {
