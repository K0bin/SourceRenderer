...
fn update_global_transforms(world: &SubWorld, command_buffer: &mut CommandBuffer) {
    let mut root_transforms_query =
        <(Entity,)>::query().filter(component::<Transform>() & !component::<Parent>());
...
fn propagate_transforms(
    ...
    let new_global_transform = if dirty {
        Some(GlobalTransform(
            *parent_transform * Matrix4::from(transform),
        ))
    } else {
        None
    };
    let global_transform = new_global_transform.as_ref().unwrap_or_else(|| {
        entry
            ...
            .expect("Parent does not have a global transform component")
    });
...
fn maintain_children(
    command_buffer: &mut CommandBuffer,
    world: &mut SubWorld,
    #[state] new_children: &mut HashMap<Entity, Vec<Entity>>,
) {
    let (ref mut children_world, ref mut other_world) = world.split::<&mut Children>();

    // handle added entities
    let mut added_parent_components_query =
        <(Entity, &Parent)>::query().filter(!component::<PreviousParent>());
    ...
    let mut changed_parent_components_query =
        <(Entity, &Parent, &PreviousParent)>::query().filter(maybe_changed::<Parent>());
    ...
    let mut children_query = <&mut Children>::query().filter(component::<Transform>());
    for children in children_query.iter_mut(children_world) {
        children
            .0
            .retain(|child| other_world.entry_ref(*child).is_ok());
    }

    // remove children component when entity has no transform
    let mut children_no_transform_query =
        <(Entity, &Children)>::query().filter(!component::<Transform>());
    ...
    let mut parent_no_transform_query =
        <(Entity, &Parent)>::query().filter(!component::<Transform>());
