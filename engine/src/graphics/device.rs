...
impl Device {
    ...
    #[inline(always)]
    pub fn create_texture(&self, info: &TextureInfo, name: Option<&str>) -> Result<Arc<super::Texture>, OutOfMemoryError> {
        super::Texture::new(&self.device, &self.allocator, &self.destroyer, info, name)
    }

    #[inline(always)]
    pub fn create_texture_view(&self, texture: &Arc<super::Texture>, info: &TextureViewInfo, name: Option<&str>) -> Arc<super::TextureView> {
        super::TextureView::new(&self.device, &self.destroyer, texture, info, name)
    }

    #[inline(always)]
    pub fn create_buffer(&self, info: &BufferInfo, memory_usage: MemoryUsage, name: Option<&str>) -> Result<Arc<BufferSlice>, OutOfMemoryError> {
    ...
    #[inline(always)]
    pub fn create_shader(&self, shader: &PackedShader, name: Option<&str>) -> active_gpu_backend::Shader {
        unsafe { self.device.create_shader(shader, name) }
    }

    #[inline(always)]
    pub fn create_graphics_pipeline(&self, info: &active_gpu_backend::GraphicsPipelineInfo, name: Option<&str>) -> Arc<super::GraphicsPipeline> {
    ...
    #[inline(always)]
    pub fn create_raytracing_pipeline(&self, info: &active_gpu_backend::RayTracingPipelineInfo, name: Option<&str>) -> Result<Arc<super::RayTracingPipeline>, OutOfMemoryError> {
    ...
    pub fn init_buffer<T>(&self, data: &[T], dst: &Arc<BufferSlice>, dst_offset: u64) -> Result<(), OutOfMemoryError> {
        let data_u8 = into_bytes(data);
        self.transfer.init_buffer(data_u8, dst, dst_offset)?;
        Ok(())
    }

    pub fn init_buffer_box<T>(&self, data: Box<[T]>, dst: &Arc<BufferSlice>, dst_offset: u64) -> Result<(), OutOfMemoryError> {
    ...
    pub fn init_texture_async<T>(
        ...
        dst: &Arc<super::Texture>,
        mip_level: u32,
        array_layer: u32) -> Result<Option<SharedFenceValuePair>, OutOfMemoryError> {
        let data_u8 = into_bytes(data);
    ...
    pub fn insert_texture_into_bindless_heap(&self, texture: &Arc<super::TextureView>) -> Option<BindlessSlot> {
    ...
    pub fn present(&self, queue_type: QueueType, swapchain: &Arc<Mutex<Swapchain>>, backbuffer: Arc<active_gpu_backend::Backbuffer>) {
