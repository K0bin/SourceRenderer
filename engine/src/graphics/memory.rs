...
impl MemoryAllocator {
    ...
    fn allocate_by_memory_type(
        ...
        let allocation = chunk_list
            .iter()
            .find_map(|chunk| chunk.allocate(size, alignment));
    ...
    pub(super) fn allocate(
        &self,
        usage: MemoryUsage,
        requirements: &gpu::ResourceHeapInfo,
    ) -> Result<MemoryAllocation<active_gpu_backend::Heap>, OutOfMemoryError> {
        let mut mask: u32;

        if usage != MemoryUsage::GPUMemory {
            mask = self.find_memory_type_mask(usage, MemoryTypeMatchingStrictness::Strict)
                & requirements.memory_type_mask;
            if let Ok(allocation) =
                self.try_allocate(mask, requirements.size, requirements.alignment)
            {
                return Ok(allocation);
            }
        }

        mask = self.find_memory_type_mask(usage, MemoryTypeMatchingStrictness::Normal)
            & requirements.memory_type_mask;
        if let Ok(allocation) = self.try_allocate(mask, requirements.size, requirements.alignment) {
            return Ok(allocation);
        }

        mask = self.find_memory_type_mask(usage, MemoryTypeMatchingStrictness::Fallback)
            & requirements.memory_type_mask;
        if let Ok(allocation) = self.try_allocate(mask, requirements.size, requirements.alignment) {
            return Ok(allocation);
        }
    ...
    pub(super) fn find_memory_type_mask(
        &self,
        usage: MemoryUsage,
        ...
        for i in 0..memory_types.len() {
            ...
            let memory_kind = if !self.is_uma()
                && (usage == MemoryUsage::GPUMemory || usage == MemoryUsage::MappableGPUMemory)
            {
                gpu::MemoryKind::VRAM
            } else {
                gpu::MemoryKind::RAM
