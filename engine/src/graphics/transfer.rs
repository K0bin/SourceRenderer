...
impl Transfer {
    ...
    pub fn init_texture_from_buffer(
        ...
        guard.graphics.copies.push(TransferCopy::BufferToImage {
    ...
    pub fn init_buffer_from_buffer(
        ...
        let actual_length = length
            .min(src_buffer.length() - src_offset)
            .min(dst_buffer.length() - dst_offset);
    ...
    pub fn init_buffer(
        ...
        let src_buffer = self.upload_data(
            data,
            dst_buffer.length() - dst_offset,
    ...
    pub fn copy_to_host_visible_texture(
        ...
        unsafe {
            self.device.transition_texture(
                texture.handle(),
    ...
    pub fn init_texture_from_buffer_async(
        ...
        let fence_value_pair = {
            ...
            transfer.copies.push(TransferCopy::BufferToImage {
                src: src_buffer.clone(),
                dst: texture.clone(),
                region: BufferTextureCopyRegion {
                    buffer_offset: buffer_offset + src_buffer.offset(),
    ...
    fn flush_commands(
        ...
        let mut cmd_buffer = if reuse_first_graphics_buffer {
            ...
            Box::new({
                TransferCommandBuffer::new(&self.device, &mut commands.pool, &commands.fence_value)
            })
        };
        ...
        cmd_buffer
            .used_buffers_slices
            .extend(commands.used_buffers_slices.drain(..));
        cmd_buffer
            .used_textures
    ...
    fn upload_data<T>(
        ...
        let size = align_up(
            if length == 0 {
                ...
                required_size.min(length as usize)
            },
            256,
        );
