...
impl<'a> CommandBuffer<'a> {
    ...
    pub fn draw_indexed(
        ...
        unsafe {
            self.cmd_buffer_handle.draw_indexed(
                index_count,
                instance_count,
                first_index,
                vertex_offset,
    ...
    pub fn bind_sampling_view(
        ...
        unsafe {
            self.cmd_buffer_handle
                .bind_sampling_view(frequency, binding, texture.handle());
        }
    }
    ...
    pub fn bind_storage_view_array(
        ...
        frequency: BindingFrequency,
        binding: u32,
        textures: &[&super::TextureView],
    ) {
        let handles: SmallVec<[&active_gpu_backend::TextureView; 4]> =
            textures.iter().map(|texture| texture.handle()).collect();

        unsafe {
            self.cmd_buffer_handle
                .bind_storage_view_array(frequency, binding, &handles);
        }
    }
    ...
    pub fn bind_storage_texture(
        ...
        unsafe {
            self.cmd_buffer_handle
                .bind_storage_texture(frequency, binding, texture.handle());
        }
    }
    ...
    pub fn bind_sampler(
        ...
        unsafe {
            self.cmd_buffer_handle
                .bind_sampler(frequency, binding, sampler.handle());
        }
    }
    ...
    pub fn bind_acceleration_structure(
        ...
        unsafe {
            self.cmd_buffer_handle.bind_acceleration_structure(
                frequency,
                binding,
    ...
    pub fn split_barrier_signal(&mut self, split_barrier: &SplitBarrier, barrier: &Barrier) {
        let core_barrier = barrier_to_backend(barrier, self.frame());
        unsafe {
            self.cmd_buffer_handle
                .split_barrier_signal(split_barrier.handle(self.frame()), &core_barrier)
        };
    }
    pub fn split_barrier_wait(&mut self, waits: &[SplitBarrierWait]) {
        let mut core_barriers =
            SmallVec::<[active_gpu_backend::Barrier; 4]>::with_capacity(waits.len());
        let mut core_waits =
            SmallVec::<[gpu::SplitBarrierWait<'_, active_gpu_backend::Backend>; 4]>::with_capacity(
                waits.len(),
            );
        ...
        unsafe {
            self.cmd_buffer_handle.split_barrier_wait(&core_waits);
        }
    }
    ...
    fn begin_render_pass_impl(&mut self, renderpass_info: &RenderPassBeginInfo, recording_mode: RenderpassRecordingMode) -> Option<<active_gpu_backend::CommandBuffer as gpu::CommandBuffer<active_gpu_backend::Backend>>::CommandBufferInheritance>{
        ...
        let attachments: SmallVec<[active_gpu_backend::RenderTarget; 5]> = renderpass_info
            .render_targets
            .iter()
            .map(|a| gpu::RenderTarget {
                view: a.view.handle(),
                load_op: a.load_op,
                store_op: match &a.store_op {
                    StoreOp::Store => gpu::StoreOp::Store,
                    StoreOp::DontCare => gpu::StoreOp::DontCare,
                    StoreOp::Resolve(resolve_attachment) => {
                        gpu::StoreOp::Resolve(gpu::ResolveAttachment {
        ...
        let depth_stencil =
            renderpass_info
                .depth_stencil
                .as_ref()
                .map(|a| gpu::DepthStencilAttachment {
                    view: a.view.handle(),
                    load_op: a.load_op,
                    store_op: match &a.store_op {
                        StoreOp::Store => gpu::StoreOp::Store,
    ...
    pub fn create_bottom_level_acceleration_structure(
        &mut self,
        info: &BottomLevelAccelerationStructureInfo,
        ...
        let (scratch, scratch_offset) = if use_preallocated_scratch {
            ...
            if reset_scratch_bump_alloc {
                ...
                unsafe {
                    self.cmd_buffer_handle
                        .barrier(&[gpu::Barrier::BufferBarrier {
    ...
    pub fn create_top_level_acceleration_structure(
        ...
        let core_instances: SmallVec<[active_gpu_backend::AccelerationStructureInstance; 16]> =
            info.instances
                .iter()
                .map(|i| gpu::AccelerationStructureInstance {
    ...
    pub fn begin_query(&mut self, query_index: u32) {
        ...
        unsafe {
            self.cmd_buffer_handle
                .begin_query(query_range.query_index(query_index));
        }
    }
    ...
    pub fn end_query(&mut self, query_index: u32) {
        ...
        unsafe {
            self.cmd_buffer_handle
                .end_query(query_range.query_index(query_index));
        }
    }
