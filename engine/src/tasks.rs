...
mod tasks_impl {
    ...
    pub fn spawn_compute_gpu_maybe_sendable<T>(future: impl Future<Output = T> + GPUMaybeSend + 'static) -> Task<T>
    where
        T: Send + 'static,
    {
    ...
    #[cfg(feature = "non_send_gpu")]
    pub fn spawn_async_compute_gpu_maybe_sendable<T>(future: impl Future<Output = T> + GPUMaybeSend + 'static) -> Task<T>
    where
        T: Send + 'static,
    {
