...
use sourcerenderer_core::graphics::{Backend, BarrierSync, BarrierAccess, TextureLayout, CommandBuffer, Barrier, Device, TextureInfo, BufferInfo, MemoryUsage, Texture, Buffer, SamplerInfo, Filter, AddressMode, BarrierTextureRange};
...
impl<B: Backend> RendererResources<B> {
  ...
  pub fn create_texture_view(&mut self, texture_name: &str, view_info: &TextureInfo) {
    let texture = self.textures.get(texture_name).expect("Texture does not exist");
    let a_mut = texture.a.borrow_mut();
    a_mut.texture.create_view(view_info, None);
    if let Some(b) = texture.b {
      let b_mut = b.borrow_mut();
      b_mut.texture.create_view(view_info, None);
    }
  }
