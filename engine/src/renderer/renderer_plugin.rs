...
fn extract_static_renderables(
    ...
    for (entity, renderable, transform) in static_renderables.iter() {
        if renderable.is_added() || transform.is_added() {
            let result = renderer.sender.register_static_renderable(
                entity,
                transform.as_ref(),
                renderable.as_ref(),
            );
...
fn extract_directional_lights(
    ...
    for (entity, light, transform) in directional_lights.iter() {
        if light.is_added() || transform.is_added() {
            let result = renderer.sender.register_directional_light(
                entity,
                transform.as_ref(),
                light.as_ref(),
            );
            ...
            async move |data| {
                ...
                *render_function.borrow_mut() = Some(Closure::new(move || {
                    ...
                    if result == EngineLoopFuncResult::Exit {
                        ...
                        let render_function_ref: &Closure<dyn FnMut()> =
                            render_function_borrow.as_ref().unwrap();
                        let id = c_scope
                            .request_animation_frame(render_function_ref.as_ref().unchecked_ref())
                            .unwrap();
