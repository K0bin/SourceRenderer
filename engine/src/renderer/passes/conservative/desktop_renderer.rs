...
pub struct ConservativeRenderer<P: Platform> {
  ...
  prepass: Prepass<P::GraphicsBackend>,
  geometry: GeometryPass<P::GraphicsBackend>,
  taa: TAAPass<P::GraphicsBackend>,
  sharpen: SharpenPass<P::GraphicsBackend>,
  ssao: SsaoPass<P>,
  occlusion: OcclusionPass<P::GraphicsBackend>,
...
impl<P: Platform> ConservativeRenderer<P> {
  pub fn new(device: &Arc<<P::GraphicsBackend as Backend>::Device>, swapchain: &Arc<<P::GraphicsBackend as Backend>::Swapchain>, _shader_manager: &mut ShaderManager<P>) -> Self {
    ...
    let clustering = ClusteringPass::<P::GraphicsBackend>::new::<P>(device, &mut barriers);
    let light_binning = LightBinningPass::<P::GraphicsBackend>::new::<P>(device, &mut barriers);
    let prepass = Prepass::<P::GraphicsBackend>::new::<P>(device, swapchain, &mut barriers);
    let geometry = GeometryPass::<P::GraphicsBackend>::new::<P>(device, swapchain, &mut barriers);
    let taa = TAAPass::<P::GraphicsBackend>::new::<P>(device, swapchain, &mut barriers, false);
    let sharpen = SharpenPass::<P::GraphicsBackend>::new::<P>(device, swapchain, &mut barriers);
    let ssao = SsaoPass::<P::GraphicsBackend>::new::<P>(device, resolution, &mut barriers, false);
    let occlusion = OcclusionPass::<P::GraphicsBackend>::new::<P>(device);
...
impl<P: Platform> RenderPath<P> for ConservativeRenderer<P> {
  ...
  fn render(
    ...
    self.occlusion.execute(&mut cmd_buf, &self.barriers, &self.device, frame_info.frame, &late_latching_buffer, scene, Prepass::<P::GraphicsBackend>::DEPTH_TEXTURE_NAME);
    ...
    self.ssao.execute(&mut cmd_buf, &self.barriers, Prepass::<P::GraphicsBackend>::DEPTH_TEXTURE_NAME, Some(Prepass::<P::GraphicsBackend>::MOTION_TEXTURE_NAME), &late_latching_buffer, self.blue_noise.frame(frame_info.frame), self.blue_noise.sampler(), shader_manager, false);
    if let Some(rt_passes) = self.rt_passes.as_mut() {
      rt_passes.shadows.execute(&mut cmd_buf, &self.barriers, Prepass::<P::GraphicsBackend>::DEPTH_TEXTURE_NAME, rt_passes.acceleration_structure_update.acceleration_structure(), &self.blue_noise.frame(frame_info.frame), &self.blue_noise.sampler());
    }
    self.geometry.execute(&mut cmd_buf, &self.barriers, &self.device, Prepass::<P::GraphicsBackend>::DEPTH_TEXTURE_NAME, scene, &frame_bindings, zero_textures, scene.lightmap.unwrap());
    self.taa.execute(&mut cmd_buf, &self.barriers, GeometryPass::<P::GraphicsBackend>::GEOMETRY_PASS_TEXTURE_NAME, Prepass::<P::GraphicsBackend>::DEPTH_TEXTURE_NAME, Some(Prepass::<P::GraphicsBackend>::MOTION_TEXTURE_NAME), false);
