    ...
    pub fn execute(
        ...
        for list in &draw.draw_lists {
            command_buffer.set_index_buffer(
                BufferRef::Regular(&list.index_buffer),
                0,
                IndexFormat::U32,
            ); //if std::mem::size_of::<imgui::DrawIdx>() == 2 { IndexFormat::U16 } else { IndexFormat::U32 });
            command_buffer.set_vertex_buffer(0, BufferRef::Regular(&list.vertex_buffer), 0);

            for draw in &list.draws {
                command_buffer.set_scissors(&[draw.scissor.clone()]);
                ...
                if let Some(texture) = &draw.texture {
                    ...
                    command_buffer.bind_sampling_view_and_sampler(
                        BindingFrequency::VeryFrequent,
                        0,
                        &pass_params.assets.get_placeholder_texture_white().view,
                        pass_params.resources.linear_sampler(),
                    );
                }
