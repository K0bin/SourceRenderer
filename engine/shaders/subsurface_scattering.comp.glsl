...
void main() {
	...
	float strength = max(1.0 - base_color.a, 0.0);
	...
	if (strength > 0.0) {
		...
		if (params.orthogonal) {
			depth = -(depth * (camera.zFar - camera.zFar) - (camera.zFar + camera.zNear)) / 2.0;
			depth_scale = params.unit_size; //remember depth is negative by default in OpenGL
		} else {
			depth = 2.0 * camera.zNear * camera.zFar / (camera.zFar + camera.zNear + depth * (camera.zFar - camera.zNear));
