...
void main() {
  ...
  barrier();
  if (true /*subgroupElect()*/) {
    //visibleBitmasks[gl_WorkGroupID.x] = visible[0];
    //visibleBitmasks[gl_WorkGroupID.x + 1] = visible[1];
    visibleBitmasks[gl_WorkGroupID.x] = ~0;
    visibleBitmasks[gl_WorkGroupID.x + 1] = ~0;
  }
}
