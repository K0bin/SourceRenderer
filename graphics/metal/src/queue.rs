...
impl MTLQueue {
    pub(crate) unsafe fn new(device: &ProtocolObject<dyn objc2_metal::MTLDevice>, shared: &Arc<MTLShared>) -> Self {
    ...
    unsafe fn submit(&self, submissions: &[gpu::Submission<MTLBackend>]) {
        ...
        for submission in submissions {
            for cmd_buf in submission.command_buffers {
                ...
                fence_wait_cmd_buffer.encodeWaitForEvent_value(&self.global_order_event, counter_val);
                ...
                if submission.command_buffers.len() == 1 {
                    let mut fences: SmallVec<[(Retained<ProtocolObject<dyn objc2_metal::MTLSharedEvent>>, u64); 4]> = SmallVec::<[(Retained<ProtocolObject<dyn objc2_metal::MTLSharedEvent>>, u64); 4]>::with_capacity(submission.signal_fences.len());
                    ...
                    cmd_buf.handle().encodeSignalEvent_value(&self.global_order_event, counter_val + 1);

                    let callback = move |_cmd_buffer: NonNull<ProtocolObject<dyn objc2_metal::MTLCommandBuffer>>| {
                    ...
                    cmd_buf.handle().addCompletedHandler(RcBlock::as_ptr(&block));
                } else {
                    cmd_buf.handle().encodeSignalEvent_value(cmd_buf.post_event_handle(), 1);
                }
                ...
                *waiting_for_completion |= 1 << (id as u64);
                let block = RcBlock::new(move |_cmd_buffer: NonNull<ProtocolObject<dyn objc2_metal::MTLCommandBuffer>>| {
            ...
            if submission.command_buffers.len() > 1 {
                ...
                for cmd_buf in submission.command_buffers {
                    fence_signal_cmd_buffer.encodeWaitForEvent_value(cmd_buf.post_event_handle(), 1);
                }
                for gpu::FenceValuePairRef { fence, value, sync_before: _ } in submission.signal_fences {
                    fence_signal_cmd_buffer.encodeSignalEvent_value(fence.event_handle(), *value);
                }
                fence_signal_cmd_buffer.encodeSignalEvent_value(&self.global_order_event, counter_val + 1);
