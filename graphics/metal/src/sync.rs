...
impl MTLFence {
    ...
    pub(crate) fn event_handle(&self) -> &ProtocolObject<dyn objc2_metal::MTLEvent> {
        match &self.event {
            MTLEventType::Regular(event) => ProtocolObject::from_ref::<
                ProtocolObject<dyn objc2_metal::MTLEvent>,
            >(event.as_ref()),
            MTLEventType::Shared(event) => ProtocolObject::from_ref::<
                ProtocolObject<dyn objc2_metal::MTLSharedEvent>,
            >(event.as_ref()),
        }
    }
...
impl gpu::Fence for MTLFence {
    ...
    unsafe fn await_value(&self, value: u64) {
        let timeout = u64::MAX;
        match &self.event {
            MTLEventType::Shared(event) => {
                event.waitUntilSignaledValue_timeoutMS(value, timeout);
            }
            _ => panic!("Fence is not CPU accessible"),
        }
    }
