...
impl MTLHeap {
    pub(crate) unsafe fn new(
        ...
        memory_type_index: u32,
        cached: bool,
        memory_kind: gpu::MemoryKind,
        mut options: objc2_metal::MTLResourceOptions,
        ...
        descriptor.setCpuCacheMode(if cached {
            objc2_metal::MTLCPUCacheMode::DefaultCache
        } else {
            objc2_metal::MTLCPUCacheMode::WriteCombined
        });
...
impl Drop for MTLHeap {
    fn drop(&mut self) {
        ...
        let index = list.iter().enumerate().find_map(|(index, heap)| {
            if heap == &self.heap {
                Some(index)
            } else {
                None
