        ...
        let (buffer, heap) = match memory {
            ResourceMemory::Dedicated {
                    ...
                    .contains(gpu::BufferUsage::ACCELERATION_STRUCTURE)
                {
                    ...
                    let buffer_opt = device.newBufferWithLength_options(
                        info.size as NSUInteger,
                        options | memory_options,
                    );
            ...
            ResourceMemory::Suballocated { memory, offset } => {
                ...
                let buffer_opt = memory.handle().newBufferWithLength_options_offset(
                    info.size as NSUInteger,
                    options,
