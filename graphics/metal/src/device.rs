...
impl MTLDevice {
    pub(crate) unsafe fn new(
        ...
        let bindless =
            MTLBindlessArgumentBuffer::new(&device, gpu::BINDLESS_TEXTURE_COUNT as usize);
    ...
    pub(crate) fn resource_options_for_memory_type(
        &self,
        memory_type_index: u32,
    ) -> objc2_metal::MTLResourceOptions {
    ...
    pub(crate) fn metal_library_from_data(
        device: &ProtocolObject<dyn objc2_metal::MTLDevice>,
        data: &[u8],
    ) -> Result<Retained<ProtocolObject<dyn objc2_metal::MTLLibrary>>, Retained<NSError>> {
...
impl gpu::Device<MTLBackend> for MTLDevice {
    ...
    unsafe fn create_heap(
        ...
        memory_type_index: u32,
        size: u64,
    ) -> Result<MTLHeap, gpu::OutOfMemoryError> {
        let memory_type = &self.memory_type_infos[memory_type_index as usize];

        let is_apple_gpu = self
            .device
            .supportsFamily(objc2_metal::MTLGPUFamily::Apple7);
    ...
    unsafe fn create_buffer(
        ...
        memory_type_index: u32,
        name: Option<&str>,
    ) -> Result<MTLBuffer, gpu::OutOfMemoryError> {
        MTLBuffer::new(
    ...
    unsafe fn get_buffer_heap_info(&self, info: &gpu::BufferInfo) -> gpu::ResourceHeapInfo {
        ...
        let size_and_align = self
            .device
            .heapBufferSizeAndAlignWithLength_options(info.size as NSUInteger, options);
        ...
        */
        ...
        let mut memory_type_mask = if !is_uma {
            1 | 1 << 1 | 1 << 2
        } else {
            1 | 1 << 1
        };
        if info
            .usage
            .contains(gpu::BufferUsage::ACCELERATION_STRUCTURE)
        {
    ...
    unsafe fn create_texture(
        ...
        memory_type_index: u32,
        name: Option<&str>,
    ) -> Result<MTLTexture, gpu::OutOfMemoryError> {
        MTLTexture::new(
            ResourceMemory::Dedicated {
                device: &self.device,
                options: self.resource_options_for_memory_type(memory_type_index),
    ...
    fn supports_mesh_shader(&self) -> bool {
        self.device.supportsFamily(objc2_metal::MTLGPUFamily::Mac2)
            || self
                .device
                .supportsFamily(objc2_metal::MTLGPUFamily::Apple7)
    }
    ...
    fn get_top_level_instances_buffer_size(
        ...
        (instances.len()
            * std::mem::size_of::<objc2_metal::MTLAccelerationStructureUserIDInstanceDescriptor>())
            as u64
    }

    unsafe fn get_raytracing_pipeline_sbt_buffer_size(
        &self,
        _info: &gpu::RayTracingPipelineInfo<MTLBackend>,
    ) -> u64 {
    ...
    unsafe fn create_raytracing_pipeline(
        &self,
        _info: &gpu::RayTracingPipelineInfo<MTLBackend>,
        _sbt_buffer: &MTLBuffer,
        _sbt_buffer_offset: u64,
        _name: Option<&str>,
    ) -> MTLRayTracingPipeline {
        panic!("The Metal backend does not support RT pipelines.")
    }
    ...
    unsafe fn transition_texture(
        &self,
        _dst: &MTLTexture,
        _transition: &gpu::CPUTextureTransition<'_, MTLBackend>,
    ) {
    }
    ...
    unsafe fn copy_to_texture(
        ...
        dst.handle()
            .replaceRegion_mipmapLevel_slice_withBytes_bytesPerRow_bytesPerImage(
                ...
                NonNull::new_unchecked(std::mem::transmute::<*const c_void, *mut c_void>(src)),
                row_pitch,
                slice_pitch,
            );
    }
