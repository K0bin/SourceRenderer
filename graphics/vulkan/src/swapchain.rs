...
use sourcerenderer_core::graphics::{SampleCount, Swapchain, SwapchainError, TextureInfo, TextureUsage, TextureSamplingViewInfo};
  ...
  fn new_internal(vsync: bool, width: u32, height: u32, device: &Arc<RawVkDevice>, surface: &Arc<VkSurface>, old_swapchain: Option<&Self>) -> Result<Arc<Self>, SwapchainError> {
    ...
    unsafe {
      ...
      let swapchain_image_views: Vec<Arc<VkTextureView>> = textures
        ...
        .enumerate()
        .map(|(index, texture)| {
          Arc::new(VkTextureView::new(device, texture, &TextureSamplingViewInfo::default(), Some(&format!("Backbuffer view {}", index))))
        })
        .collect();
