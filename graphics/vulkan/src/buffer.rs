...
impl VkBufferSlice {
    ...
    pub fn va(&self) -> Option<vk::DeviceAddress> {
        self.buffer
            .va()
            .map(|va| va + self.offset as vk::DeviceSize)
    }

    pub fn va_offset(&self, offset: usize) -> Option<vk::DeviceAddress> {
        self.buffer
            .va()
            .map(|va| va + (self.offset + offset) as vk::DeviceSize)
    }
}
...
impl BufferAllocator {
    pub fn new(device: &Arc<RawVkDevice>, reuse_automatically: bool) -> Self {
        ...
        unsafe {
            device
                .instance
                .get_physical_device_properties2(device.physical_device, &mut limits2)
        }
        ...
        let transfer_pool = if reuse_automatically {
            ...
            unsafe {
                assert_eq!(
                    vma_sys::vmaFindMemoryTypeIndexForBufferInfo(
                        device.allocator,
                        &buffer_info as *const vk::BufferCreateInfo,
                        &allocation_info as *const vma_sys::VmaAllocationCreateInfo,
                        &mut memory_type_index as *mut u32
                    ),
                    vk::Result::SUCCESS
                );
            }
            ...
            unsafe {
                ...
                let res = vma_sys::vmaCreatePool(
                    device.allocator,
                    &pool_info as *const vma_sys::VmaPoolCreateInfo,
                    &mut pool as *mut vma_sys::VmaPool,
                );
    ...
    pub fn get_slice(
        ...
        if info.size > BIG_BUFFER_SLAB_SIZE && self.reuse_automatically {
            let pool = if memory_usage == MemoryUsage::UncachedRAM
                && info.usage == BufferUsage::COPY_SRC
            {
                self.transfer_pool
            } else {
                None
        ...
        if info.usage.contains(BufferUsage::CONSTANT) {
            ...
            alignment = max(
                alignment,
                self.device_limits.min_uniform_buffer_offset_alignment as usize,
            );
        }
        if info.usage.contains(BufferUsage::STORAGE) {
            ...
            alignment = max(
                alignment,
                self.device_limits.min_storage_buffer_offset_alignment as usize,
            );
        }
        ...
        if info.usage.contains(BufferUsage::SHADER_BINDING_TABLE) {
            ...
            alignment = max(
                alignment,
                rt.rt_pipeline_properties.shader_group_handle_alignment as usize,
            );
            alignment = max(
                alignment,
                rt.rt_pipeline_properties.shader_group_base_alignment as usize,
            );
        }
        ...
        let slice_index = matching_buffers
            .free_slices
            .iter()
            .enumerate()
            .find(|(_, slice)| {
