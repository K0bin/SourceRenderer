...
pub trait PlatformIO: 'static + Send + Sync {
    ...
    fn open_asset<P: AsRef<Path> + Send>(
        path: P,
    ) -> impl Future<Output = IOResult<Self::File>> + IOMaybeSend;
    fn asset_exists<P: AsRef<Path> + Send>(path: P) -> impl Future<Output = bool> + IOMaybeSend;
    fn open_external_asset<P: AsRef<Path> + Send>(
        path: P,
    ) -> impl Future<Output = IOResult<Self::File>> + IOMaybeSend;
    fn external_asset_exists<P: AsRef<Path> + Send>(
        path: P,
    ) -> impl Future<Output = bool> + IOMaybeSend;
