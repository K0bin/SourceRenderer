...
fn main() {
    ...
    let ndk_path = build_util::android::derive_ndk_path()
        .expect("Can't find Android NDK, try setting the environment variable ANDROID_NDK_HOME.")
        .1;
    ...
    lib_path.push(
        target_mapping
            .get(target.as_str())
            .expect("Failed to map LLVM target triple to Android jniLibs directory."),
    );
    ...
    if !android_asset_dir.exists() {
        std::fs::create_dir_all(&android_asset_dir)
    ...
    if !asset_dest_dir.exists() {
        std::fs::create_dir_all(&asset_dest_dir)
            .expect("Failed to create assets target directory.");
    }
