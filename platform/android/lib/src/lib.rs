...
use ndk_sys::{
    __android_log_print, android_LogPriority, android_LogPriority_ANDROID_LOG_ERROR,
    android_LogPriority_ANDROID_LOG_INFO,
};
use sourcerenderer_core::platform::Window;
use sourcerenderer_core::Vec2UI;
use sourcerenderer_core::{platform::Event, Platform, Vec2};
...
lazy_static! {
    static ref TAG: CString = { CString::new("SourceRenderer").unwrap() };
}
...
fn engine_from_long<'a>(engine_ptr: jlong) -> RefMut<'a, EngineWrapper> {
    ...
    unsafe {
        ...
        let engine_ref =
            std::mem::transmute::<RefMut<EngineWrapper>, RefMut<'a, EngineWrapper>>(engine);
        engine_ref
    }
}
...
pub extern "system" fn Java_de_kobin_sourcerenderer_MainActivity_startEngineNative(
    ...
    let native_window_ptr = unsafe {
        ANativeWindow_fromSurface(std::mem::transmute(env), std::mem::transmute(*surface))
    };
    ...
    unsafe { std::mem::transmute(Box::into_raw(engine)) }
}
...
pub extern "system" fn Java_de_kobin_sourcerenderer_MainActivity_onSurfaceChangedNative(
    ...
    if surface.is_null() {
        ...
        if &native_window != wrapper.platform.window().native_window() {
            wrapper
                .platform
                .change_window(AndroidWindow::new(native_window));
            wrapper.engine.dispatch_event(Event::SurfaceChanged(
                wrapper
                    .platform
                    .window()
                    .create_surface(wrapper.engine.instance().clone()),
            ));
            wrapper
                .engine
                .dispatch_event(Event::WindowSizeChanged(Vec2UI::new(
                    wrapper.platform.window().width(),
                    wrapper.platform.window().height(),
                )));
        }
    }
}
...
pub extern "system" fn Java_de_kobin_sourcerenderer_MainActivity_onTouchInputNative(
    ...
    {
        match event_type {
            ANDROID_EVENT_TYPE_POINTER_DOWN | ANDROID_EVENT_TYPE_DOWN => {
            ...
            ANDROID_EVENT_TYPE_POINTER_UP | ANDROID_EVENT_TYPE_UP => {
                engine.dispatch_event(Event::FingerMoved {
                    index: finger_index as u32,
                    position: Vec2::new(x, y),
                });
