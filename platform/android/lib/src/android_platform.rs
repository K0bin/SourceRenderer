...
use ndk_sys::{ANativeWindow_getHeight, ANativeWindow_getWidth, ANativeWindow_release};
use sourcerenderer_core::platform::FileWatcher;
use sourcerenderer_core::platform::{ThreadHandle, Window};
...
impl Window<AndroidPlatform> for AndroidWindow {
    fn create_surface(&self, graphics_instance: Arc<VkInstance>) -> Arc<VkSurface> {
        ...
        let android_surface_loader =
            AndroidSurface::new(&instance_raw.entry, &instance_raw.instance);
        let surface = unsafe {
    ...
    fn width(&self) -> u32 {
        unsafe { ANativeWindow_getWidth(self.native_window.ptr().as_ptr()) as u32 }
    }

    fn height(&self) -> u32 {
        unsafe { ANativeWindow_getHeight(self.native_window.ptr().as_ptr()) as u32 }
    }
}
