...
fn main() {
  ...
  compile_shaders(&shader_dir, &shader_dir_temp, true, &HashMap::new(), ShadingLanguage::Wgsl, |f| f.extension().and_then(|os_str| os_str.to_str()).unwrap_or("") == "glsl" && f.file_stem().and_then(|ext| ext.to_str()).map(|s| s.contains(".web.")).unwrap_or(false));
  ...
  contents
    ...
    .for_each(|file| {
      ...
      unsafe {
        ...
        assert_eq!(spirv_cross_sys::spvc_context_parse_spirv(context, words.as_ptr() as *const u32, words_len as usize, &mut ir), spirv_cross_sys::spvc_result_SPVC_SUCCESS);
