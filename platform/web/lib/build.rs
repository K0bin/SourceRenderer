...
fn main() {
  ...
  compile_shaders(&shader_dir, &shader_dir_temp, true, false, &HashMap::new(), |f| f.extension().and_then(|os_str| os_str.to_str()).unwrap_or("") == "glsl" && f.file_stem().and_then(|ext| ext.to_str()).map(|s| s.contains(".web.")).unwrap_or(false));
